...
class productClass:
    def __init__(self,root):
        ...
        product_Frame=Frame(self.root,bd=2,relief=RIDGE,bg="lightgrey")
        product_Frame.place(x=10,y=10,width=480,height=520)
      #========title==================
        title=Label(product_Frame,text=" Manage Product Details",font=("goudy old style",18),bg="#0f4d7d",fg="white").pack(side=TOP,fill=X)
    #===============column1==================
        lbl_category=Label(product_Frame,text="Category",font=("goudy old style",18),bg="lightgrey").place(x=30,y=60)
        lbl_supplier=Label(product_Frame,text="Supplier",font=("goudy old style",18),bg="lightgrey").place(x=30,y=120)
        lbl_product_name=Label(product_Frame,text="Product",font=("goudy old style",18),bg="lightgrey").place(x=30,y=180)
        lbl_price=Label(product_Frame,text="Price",font=("goudy old style",18),bg="lightgrey").place(x=30,y=240)
        lbl_Quantity=Label(product_Frame,text="Quantity",font=("goudy old style",18),bg="lightgrey").place(x=30,y=300)
        lbl_status=Label(product_Frame,text="Status",font=("goudy old style",18),bg="lightgrey").place(x=30,y=360)
    ...
    #================column 2==================
        ...
        cmb_status.current(0)
         ...
         #============button============
        btn_add=Button(product_Frame,text="Save",command=self.add,font=("goudy old style",15),bg="#2196f3",fg="white",cursor="hand2").place(x=10,y=430,width=100,height=40)
        btn_update=Button(product_Frame,text="Update",command=self.update,font=("goudy old style",15),bg="#4caf50",fg="white",cursor="hand2").place(x=120,y=430,width=100,height=40)
        btn_delete=Button(product_Frame,text="Delete",command=self.delete,font=("goudy old style",15),bg="#f44336",fg="white",cursor="hand2").place(x=230,y=430,width=100,height=40)
        btn_clear=Button(product_Frame,text="Clear",command=self.clear,font=("goudy old style",15),bg="#607b8b",fg="white",cursor="hand2").place(x=340,y=430,width=100,height=40)
        
        #============search===========
        SearchFrame=LabelFrame(self.root,text="Search Employee",font=("goudy old style",12,"bold"),bd=2,relief=RIDGE,bg="lightgrey")
        SearchFrame.place(x=520,y=20,width=600,height=80)
        ...
        p_frame.place(x=519,y=100,width=600,height=427)
        ...
        try:
            if self.var_cat.get()=="Select" or self.var_sup.get()=="Empty" or self.var_name.get()=="":
                ...
                if row!=None:
                    ...
                    con.commit()
                    messagebox.showinfo("Success","Product Addeed Successfully",parent=self.root)
                    self.show()
        except Exception as ex:
            messagebox.showerror("Error",f"Error due to : {str(ex)}",parent=self.root)
        ...
        try:
            if self.var_pid.get()=="":
                ...
                if row==None:
                    ...
                    con.commit()
                    messagebox.showinfo("Success","Product Udated Successfully",parent=self.root)
                    self.show()
